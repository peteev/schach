...
bturm_img = ImageTk.PhotoImage(photo.resize((40, 40)))
photo = Image.open("pieces/whiterook.png")
wturm_img = ImageTk.PhotoImage(photo.resize((40, 40)))
...
# Store the images in the dictionary with the corresponding piece names as keys
img["bp"] = bpawn_img
img["wp"] = wpawn_img
img["br"] = bturm_img
img["wr"] = wturm_img
...
class Bauer(Figur):
    def __init__(self, farbe, num):
        ...
        if self.farbe == "w" and self.posY == 8:
            self.umwandlung = True
        elif self.farbe == "b" and self.posY == 1:
            self.umwandlung = True
